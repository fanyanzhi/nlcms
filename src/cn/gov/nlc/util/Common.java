...
public class Common {
	private static final Logger logger = Logger.getLogger(cn.gov.nlc.util.Common.class);

	public static String mergeTime(String str1, String str2) throws Exception {
		SimpleDateFormat sdf3 = new SimpleDateFormat("yyyy-MM-dd");
		Date dd1 = sdf3.parse(str1);
		Date dd2 = sdf3.parse(str2);
		DateTime sd = new DateTime(dd1);
		DateTime ed = new DateTime(dd2);
	...
	public static String mergeTime(Date d1, Date d2) throws Exception {
		...
		String ss1 = sdf1.format(d1);
		String ss2 = sdf1.format(d2);
		DateTime sd = new DateTime(ss1);
		DateTime ed = new DateTime(ss2);
		int days = Days.daysBetween(sd, ed).getDays();
	...
	public static String[] getTwoAddressByIP(String ip) {
		...
		if (ip.equals("127.0.0.1") || ip.equals("localhost") || ip.startsWith("192.168")) {
			res[0] = "北京";
			res[1] = "北京";
			return res;
		}
		...
		try {
			...
			String result = ProxyHttpClient.sendGet(url);
	...
	public static String sendPostFormSso(String url, com.alibaba.fastjson.JSONObject jsonObj) {
		...
		String token = getToken(timestamp, "netjson", "netpass1234");

		method.setRequestHeader("token", token);
		method.setRequestHeader("timestamp", timestamp);
		method.setRequestHeader("jsonuser", "netjson");
		method.setRequestHeader("logincode", "netpass1234");
