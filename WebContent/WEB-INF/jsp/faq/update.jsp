	...
	<form id="adminForm">
		<table align="center" width="90%" border="0">
			<tr>
				...
				<td width="40%">
					<input type="text" id="question" value="${faq.question}" style="width:300px" name="question" class="easyui-validatebox" required data-options="validType:['specialy','isBlank','length[0,200]']" />
				</td>
				...
				<td width="15%">
					<select name="status" id="status" class="easyui-combobox" data-options="width:100,height:29,editable:false,panelHeight:50">
								<option value="已发布"  <c:if test="${faq.status == '已发布'}">selected="selected"</c:if>>已发布</option>
								<option value="未发布"  <c:if test="${faq.status == '未发布'}">selected="selected"</c:if>>未发布</option>
					</select>
				</td>
				
				<td width="10%" align="right">排序：</td>
				<td width="15%">
					<input type="text" id="cseq" name="cseq" class="easyui-numberbox" data-options="required:true,max:1000,min:1" value="${faq.cseq }"/>
				</td>
			</tr>
		</table>
		<br/>
...
<script>
	...
	function saveb() {
		...
		$.ajax({
            url: "${ctx}/session/faq/updatefaq",
            data: {"id":id, "question":question, "status":status, "cseq":cseq, "answer":answer},
            cache: false,
            type: "POST",
