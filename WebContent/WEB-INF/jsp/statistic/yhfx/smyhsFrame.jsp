...
<div>
	<form>
	<font style="color:red">实名用户数量</font>
	...
	<br/>
	<div>
	<strong>实名用户数据</strong> (<span id="tjfs" value='month'>按月统计</span>)
		<div>
			<iframe id="winfra" src="about:blank" width="100%" height="400px" frameborder=0 marginheight=0></iframe>
		</div>
	</div>
		<br/><br/>
...
$(function(){
	...
	$.ajax({
		url:"${ctx}/session/statistic/smyhs1List",
		type:'POST',
		async:false,
		...
		success:function(data){
			...
			if(!data.result) {
				...
				labels = data.labels;
				executedraw();
				$("#winfra").attr("src", "${ctx}/session/statistic/smyhstable?type=month&startDate=${startDate}&endDate=${endDate}&r="+Math.random());
			}
		}
	});
	...
	$.ajax({
		...
		success:function(data){
			...
			if(!data.result) {
				...
				labels = data.labels;
				executedraw();
				$("#winfra").attr("src", "${ctx}/session/statistic/smyhstable?type="+type+"&startDate="+s+"&endDate="+e+"&r="+Math.random());
			}
		}
	});
}
		...
		tip:{	//提示框
			...
			listeners:{
				...
				parseText:function(tip,name,value,text,i){
					return "<span style='color:#005268;font-size:12px;'>"+labels[i]+"月新增实名用户数量:"+
