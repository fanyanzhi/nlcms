...
<div>
	...
	<div>
	<strong>持卡用户数据</strong>(<span id="tjfs" value='month'>按月统计</span>)
		<div>
...
$(function(){
	$.ajax({
		url:"${ctx}/session/statistic/ckyhs1List",
		type:'POST',
		async:false,
		...
		success:function(data){
			...
			if(!data.result) {
				...
				labels = data.labels;
				executedraw();
				$("#winfra").attr("src", "${ctx}/session/statistic/ckyhtable?startDate=${startDate}&endDate=${endDate}&type=month&r="+Math.random());
			}
		}
	});
})
	...
	$.ajax({
		...
		success:function(data){
			...
			if(!data.result) {
				...
				labels = data.labels;
				executedraw();
				$("#winfra").attr("src", "${ctx}/session/statistic/ckyhtable?type="+type+"&startDate="+s+"&endDate="+e+"&r="+Math.random());
			}
		}
	});
}
		...
		tip:{	//提示框
			...
			listeners:{
				...
				parseText:function(tip,name,value,text,i){
					return "<span style='color:#005268;font-size:12px;'>"+labels[i]+"月新增持卡用户数量:"+
