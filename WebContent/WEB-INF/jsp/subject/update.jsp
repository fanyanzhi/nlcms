	...
	<form id="adminForm">
		...
		<table align="center" width="90%" >
			<tr>
				...
				<td width="20%">
					<input type="text" id="title" style="width:210px" name="title" value="${nlcsubject.title }" class="easyui-validatebox" required data-options="validType:['specialy','isBlank','length[0,330]']" />
				</td>
				
				<td width="20%" align="right">标题图片：<br/><span style="font-size:12px;color:#ccc">(最大5M)</span>&nbsp;&nbsp;&nbsp;</td>
				<td width="20%">
					<input type="button" id="up" value="上传图片" ><span id="s1"></span>
				</td>
...
<script>
	...
	$(function(){
		...
		$("input[name='pushmethod']").each(function(index, obj){
			var strArr = "${nlcsubject.pushmethod}".split(",");
		...
		//$("#pubtime").datetimebox('setValue', "<fmt:formatDate value='${nlcsubject.pubtime}' pattern='yyyy-MM-dd HH:mm:ss'/>");
		...
		$("#saveBtn").click(function(){
			...
			$.ajax({
                url: "${ctx}/session/subject/updateSubject",
                data: {"pic":pic, "introduce":introduce, "title":title, "id":"${nlcsubject.id}", "pushmethod":parr.join()},
                cache: false,
                type: "POST",
		...
		ue.addListener("ready", function () {
			var s = '${nlcsubject.introduce}'.replace(/\"/g,"'"); 
			//s = s.replace(/[\n]/g, "");
			ue.setContent(s);
		});
