...
<!-- header 新闻详情页-->
    ...
    <div class="contentbox" >
    	<div class="contitle">
        	<h1>${nlcnotice.title }</h1>
            <span><fmt:formatDate value='${nlcnotice.pubTime }' pattern='yyyy-MM-dd HH:mm'/></span>
        </div>
        <div class="content">
        	<p>${nlcnotice.content }</p>
...
<script>
	$(function(){
		$.ajax({
			url:"${ctx}/share/noticelist",
			type:'POST',
			data:{"page":1, "rows":5, "noticeid":"${nlcnotice.noticeid}"},
			dataType: "json",
			success:function(data){
					var arrObj = data.rows;
					var html = '';
					for(var i = 0; i < arrObj.length; i++) {
						var linktitle = '<a href="${ctx}/share/notice?noticeid='+arrObj[i].noticeid+'">'+arrObj[i].title+'</a>';
						html += '<div class="zhuant">'+linktitle
								+'<p>'
								+'<em>'+arrObj[i].pubTime+'</em>&nbsp;&nbsp;&nbsp;'
								+'<b>'+arrObj[i].praisecount+'</b>'
								+'</p>'
								+'</div>';
					}
					$("#sign").append(html);
    ...
    <div id="sign" class="xgbox clearfix">
        ...
        <!--第一条-->
       <!--  <div class="zhuant">
                <a href="">标题内容标题内容标题内容标题内容标题内容标题内容
                标题内容标题内容标题内容标题内容标题内容标题内容</a>
                <p>
