...
<font style="font-family:黑体">活跃用户</font>
<br/>
<table border="0">
		<tr>
			<td>&nbsp;</td>
	...
	<br/>
	<font id="tit1" style="font-family:黑体">按天统计(当天启动的用户数去重)</font>
	<div>
...
<script>
var tit1 = "按天统计(当天启动的用户数去重)";

$(function(){
	$("#winfra").attr("src", "${ctx}/session/statisticy/hyyhGrid?status=day&startDate=${lastmonth}&endDate=${yesterday}&r="+Math.random());
});
...
function staday() {
	...
	tit1 = "按天统计";
	$("#tit1").html("按天统计(当天启动的用户数去重)");
	
	$("#winfra").attr("src", "about:blank");
	$("#winfra").attr("src", "${ctx}/session/statisticy/hyyhGrid?status=day&startDate="+ws+"&endDate="+we+"&r="+Math.random());
}
...
function stamon() {
	...
	tit1 = "按月统计";
	$("#tit1").html("按月统计(当月启动的用户数去重)");

	$("#winfra").attr("src", "about:blank");
	$("#winfra").attr("src", "${ctx}/session/statisticy/hyyhGrid?status=month&startDate="+s+"&endDate="+e+"&r="+Math.random());
}
...
function stayear() {
	...
	$("#winfra").attr("src", "about:blank");
	$("#winfra").attr("src", "${ctx}/session/statisticx/sysSearchYmfwGrid?status=year&startDate="+s+"&endDate="+e+"&r="+Math.random()); */
}
...
function staweek() {
	...
	tit1 = "按周统计";
	$("#tit1").html("按周统计(当周启动的用户数去重)");
	
	$("#winfra").attr("src", "about:blank");
	$("#winfra").attr("src", "${ctx}/session/statisticy/hyyhGrid?status=week&startDate="+ws+"&endDate="+we+"&r="+Math.random());
}
