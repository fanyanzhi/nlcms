...
<script type="text/javascript">
	...
	$(function(){
		//结束时间大于开始时间
		checkEndDate('startWjdate','endWjdate');
		//时间框禁止输入
		$(".datebox :text").attr("readonly","readonly");
		
		tab = $('#dataTable').datagrid({
			//数据来源
			url:'${ctx}/session/wjreader/list',
			//斑马纹
			striped:true,
			//主键字段
			idField:"id",
	...
	function deleteInfo(id, name) {
		if(confirm('确定要删除？')){
			$.ajax({
			url:"${ctx}/session/wjreader/delete/" + id +"?r=" + Math.random(),
			type:'GET',
	...
	function deleteAll() {
		...
		if(confirm('确定要删除？')) {
			...
			for(var i = 0; i < rows.length; i++) {
				...
				nameArr.push(rows[i].shiju);
			}
			
			$.ajax({
				url:"${ctx}/session/wjreader/deleteAll?r=" + Math.random(),
				data: "ids=" + idArr.join(),
				type:'POST',
				success:function(data){
						insertLog("删除文津 " + nameArr.join());
						$('#dataTable').datagrid("reload");
					}
				});
		}else {
	...
	$(function(){
		$("#queryBtn").click(function(){
			var startWjdate = $("#startWjdate").datebox("getValue");
			var endWjdate = $("#endWjdate").datebox("getValue");
			
			tab.datagrid("load", {
				"quanshi" : $("#quanshi").val(),
				"geyan" : $("#geyan").val(),
				"startWjdate" : startWjdate,
				"endWjdate" : endWjdate,
				//状态
				"status" : $("#status").val(),
				//来源
		...
		$("#resetBtn").click(function(){
			...
			$("#startWjdate").datebox("setValue");
			$("#endWjdate").datebox("setValue");
			$("#queryBtn").click();
		})
	...
	function previewInfo(id) {
		...
		if (navigator.userAgent.indexOf('Firefox') >= 0){
			window.showModalDialog ('${ctx}/session/wjreader/itemPreview?id=' + id, '预览', 'dialogHeight=680px; dialogWidth=340px');
		} else {
			window.open('${ctx}/session/wjreader/itemPreview?id=' + id, null,"height=680,width=340,status=yes,toolbar=no,menubar=no,location=no");
		}
	}
	...
	function operateFun(id, status, title) {
		...
		if(confirm('确定要'+confirmword+'？')){
			$.ajax({
				url:"${ctx}/session/wjreader/publish?id="+id+"&status="+status+"&r=" + Math.random(),
				type:'GET',
				success:function(data){
						  var res = eval("("+data+")");
						  if(res.result == false) {
							...
							insertLog("文津 "+title+" "+confirmword);
							$('#dataTable').datagrid("reload");
						  }
					} 
				});
	...
	function updateInfo(id) {
		$('#dataTable').datagrid("clearSelections");
		openWin2("修改", "${ctx}/session/wjreader/update?id="+id, 1000, 450);
	}
	...
	function addnews() {
		$('#dataTable').datagrid("clearSelections");
		openWin2("添加", "${ctx}/session/wjreader/add", 1000, 450);
	}
...
<div>
	<form id="queryForm">
		<table style="width:80%">
			<tr>
				...
				<td colspan="3">
					<input type="text" id="startWjdate" name="startWjdate" class="easyui-datebox" /> -- 
					<input type="text" id="endWjdate" name="endWjdate" class="easyui-datebox" />
				</td>
			</tr>
			...
			<tr>
				...
				<td width="20%" align="right">诗词：</td>
				<td width="30%"><input type="text" id="quanshi" style="width:250px" name="quanshi" /></td>
			</tr>
			...
			<tr>
				...
				<td width="20%" align="right">格言：</td>
				<td width="30%"><input type="text" id="geyan" style="width:250px" name="geyan" /></td>
			</tr>
	...
	<c:if test="${LoginObj.role != 2}">
		...
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-reload',plain:true" onclick="pull()">拉取当天数据</a>
	</c:if>
	</div>
