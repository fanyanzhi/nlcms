...
<script type="text/javascript">
	...
	function deleteInfo(id, title) {
		if(confirm('确定要删除？')){
			$.ajax({
			url:"${ctx}/session/hotword/delete/" + id +"?r=" + Math.random(),
			type:'GET',
			success:function(data){
					insertLog("删除opac热词 "+title);
	...
	function deleteAll() {
		...
		if(confirm('确定要删除？')) {
			...
			$.ajax({
				url:"${ctx}/session/hotword/deleteAll?r=" + Math.random(),
				data: "ids=" + idArr.join(),
				type:'POST',
				success:function(data){
						insertLog("删除opac热词 "+titleArr.join());	
						$('#dataTable').datagrid("reload");
					}
				});
		}else {
	...
	function saveInfo() {
		...
		if(rows.length > 0) {
			...
			$.ajax({
				...
				async : false,
				type : 'post',
				url : "${ctx}/session/hotword/saveModi",
				data : {json : effectRow},
				dataType: "json",
				success : function(data){
					var res = eval("("+data+")");
					if(res.result == false) {
						...
						insertLog("修改opac热词 " +wordarr.join());
						tab.datagrid('reload');
					}
