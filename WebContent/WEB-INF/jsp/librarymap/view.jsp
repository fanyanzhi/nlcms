...
<script type="text/javascript">
	...
	$(function(){
		...
		tab = $('#dataTable').datagrid({
			//数据来源
			url:'${ctx}/session/librarymap/list',
			//斑马纹
			striped:true,
			//主键字段
			idField:"id",
	...
	function updateInfo(id) {
		...
		openWin2("编辑", "${ctx}/session/librarymap/update?id=" + id, 800, 500);
	}
	...
	//新增
	function addObject() {
		openWin2("新增", "${ctx}/session/librarymap/add", 800, 500);
	}
	...
	function deleteInfo(id, name) {
		...
		if(confirm('确定要删除？')){
			$.ajax({
			url:"${ctx}/session/librarymap/delete/" + id +"?r=" + Math.random(),
			type:'GET',
			success:function(data){
					insertLog("删除馆区地图 "+name);
